...
namespace ProjektZaliczeniowyPBTesty
{
    ...
    public sealed class ProjektZaliczeniowyPBTesty
    {
        ...
        //Test edycji zakupu (zmiana przypisanego samochodu)
        [TestMethod]
        public void UpdateZakup_ZmieniaSamochod()
        {
            using (var db = new ProjektZaliczeniowyBazaSamochodowEntities())
            {
                var klient = db.Klienci.FirstOrDefault() ?? new Klienci { Imie = "A", Nazwisko = "B", Email = Guid.NewGuid() + "@x.pl", NumerTelefonu = "123456789" };
                if (klient.KlientID == 0) { db.Klienci.Add(klient); db.SaveChanges(); }

                var pracownik = db.Pracownicy.FirstOrDefault() ?? new Pracownicy { Imie = "Jan", Nazwisko = "Test", Email = Guid.NewGuid() + "@x.pl", Miejscowosc = "Miasto" };
                if (pracownik.PracownikID == 0) { db.Pracownicy.Add(pracownik); db.SaveChanges(); }

                var samochod1 = new Samochody { Model = "Zmiana1", RokProdukcji = 2020, NumerSeryjny = Guid.NewGuid().ToString().Substring(0, 16), WersjaWyposazenia = "A" };
                var samochod2 = new Samochody { Model = "Zmiana2", RokProdukcji = 2021, NumerSeryjny = Guid.NewGuid().ToString().Substring(0, 16), WersjaWyposazenia = "B" };
                db.Samochody.Add(samochod1); db.Samochody.Add(samochod2); db.SaveChanges();

                var zakup = new Zakupy
                {
                    KlientID = klient.KlientID,
                    PracownikID = pracownik.PracownikID,
                    SamochodID = samochod1.SamochodID,
                    DataZakupu = DateTime.Now
                };
                db.Zakupy.Add(zakup);
                db.SaveChanges();

                //Edytujemy zakup, podmieniamy samochód
                zakup.SamochodID = samochod2.SamochodID;
                db.SaveChanges();

                var edytowany = db.Zakupy.Find(zakup.ZakupID);
                Assert.AreEqual(samochod2.SamochodID, edytowany.SamochodID);

                //Ostatecznie usuwamy zakup i samochody, aby nie pozostawiać testowych danych w bazie
                db.Zakupy.Remove(edytowany);
                db.Samochody.Remove(samochod1);
                db.Samochody.Remove(samochod2);
                db.SaveChanges();
            }
        }

        //Test odpowiadający za to czy nie można dodać polisy z datą zakończenia wcześniejszą niż rozpoczęcia
        [TestMethod]
        [ExpectedException(typeof(System.Data.Entity.Infrastructure.DbUpdateException))]
        public void AddPolisa_ZlaDataZakonczenia()
        {
            using (var db = new ProjektZaliczeniowyBazaSamochodowEntities())
            {
                var zakup = db.Zakupy.FirstOrDefault();
                if (zakup != null)
                {
                    var polisa = new Polisy
                    {
                        ZakupID = zakup.ZakupID,
                        DataRozpoczecia = DateTime.Now.Date,
                        DataZakonczenia = DateTime.Now.Date.AddDays(-1) //wcześniejsza data!
                    };
        ...
        //Test odpowiedzialny za dodanie wydziału i powiązanie z pracownikiem (jeśli relacja istnieje)
        [TestMethod]
        public void AddWydzial_PrzydzielPracownika()
        {
            using (var db = new ProjektZaliczeniowyBazaSamochodowEntities())
            {
                var wydzial = new Wydzialy { Nazwa = "NowyTestWydzial" + Guid.NewGuid().ToString("N").Substring(0, 5) };
                db.Wydzialy.Add(wydzial);
                db.SaveChanges();

                var pracownik = new Pracownicy { Imie = "TestW", Nazwisko = "W", Email = Guid.NewGuid() + "@test.pl", Miejscowosc = "Test" };
                db.Pracownicy.Add(pracownik);
                db.SaveChanges();

                //Przypisanie wydziału do pracownika (jeśli relacja istnieje)
                pracownik.Wydzialy.Add(wydzial);
                db.SaveChanges();

                var zBazy = db.Pracownicy.Include("Wydzialy").FirstOrDefault(p => p.PracownikID == pracownik.PracownikID);
                Assert.IsTrue(zBazy.Wydzialy.Any(w => w.WydzialID == wydzial.WydzialID));

                //Ostatecznie usuwamy wydział i pracownika, aby nie pozostawiać testowych danych w bazie
                pracownik.Wydzialy.Remove(wydzial);
                db.Pracownicy.Remove(pracownik);
                db.Wydzialy.Remove(wydzial);
                db.SaveChanges();
            }
        }
    }
